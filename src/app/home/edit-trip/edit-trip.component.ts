...
export class EditTripComponent  implements OnInit {
  ...
  save() {
    this.formulaires.get('id').setValue(this.data[0].id);
    ...
    if (this.formulaires.invalid) {
      ...
      this.submittedTrip = true;
      this.services.updates(this.formulaires.value).subscribe({
        next: (value: any) => {
  ...
  getAllObject() {
      ...
      this.placeservice.getAll().subscribe({
        next: (value) => {
          ...
          this.temporaile = this.data[0].places

        console.log(this.data)
        console.log(this.temporaile)
        },
        error: (err) => {console.error('Observable emitted an error: ' + err), this.submitteds = false},
        complete: () => (this.submitteds = false),
      });
  }
  enregistrer() {
